...
func popup_in_position(menu : MenuButton):
	...
	menu.get_popup().rect_position.x = menu.get_global_transform_with_canvas().origin.x
	menu.get_popup().rect_position.y = menu.get_global_transform_with_canvas().origin.y + menu.rect_size.y
...
func get_maxpopup_bounds(popup : PopupMenu) -> Vector2:
	var max_size = popup.rect_size + popup.get_global_transform_with_canvas().origin
...
func is_in_bounds(target : Control, pos : Vector2) -> bool:
	var min_pos = target.get_global_transform_with_canvas().origin
